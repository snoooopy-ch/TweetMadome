...
export class LeftPanelComponent implements OnInit, OnDestroy {
  ...
  setHotKeys() {
    ...
    if (this.subHotKeys.hasOwnProperty('key_container1')) {
      ...
      // レス描写エリアの一番下に移動
      this.hotkeysService.add(new Hotkey([this.subHotKeys['scroll_most_down'], this.subHotKeys['scroll_most_down2'], 'ctrl+end'],
      ...
      // 「上余白」のチェックON/OFF
      this.hotkeysService.add(new Hotkey(this.subHotKeys['ue_yohaku'], (event: KeyboardEvent): boolean => {
        if (this.hovered >= 0) {
          this.twitList[this.hovered].isAddTopLink = !this.twitList[this.hovered].isAddTopLink;
        }
        return false; // Prevent bubbling
      }));

      // 「下余白」のチェックON/OFF
      this.hotkeysService.add(new Hotkey(this.subHotKeys['shita_yohaku'], (event: KeyboardEvent): boolean => {
        if (this.hovered >= 0) {
          this.twitList[this.hovered].isAddBottomLink = !this.twitList[this.hovered].isAddBottomLink;
        }
        return false; // Prevent bubbling
      }));
